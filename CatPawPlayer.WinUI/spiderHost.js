...
// Dynamic custom spider engines registry: { [hash]: { port, app, started } }
...
// 1. Start Erxiao Engine (Port 9988)
async function startErxiao() {
  if (erxiaoStarted) return;
  try {
    if (!fs.existsSync(erxiaoJs)) {
      console.log('[SpiderHost] Downloading Erxiao script...');
...
// 2. Start Douer Engine (Port 2333)
async function startDouer() {
  if (douerStarted) return;
  try {
    if (!fs.existsSync(douerJs)) {
      console.log('[SpiderHost] Downloading Douer script...');
        ...
        try {
          ...
          if (subUrl.endsWith('.md5') || subUrl.endsWith('.js') || rawText.includes('websiteBundle') || rawText.includes('catServerFactory')) {
            ...
            if (jsContent.length > 5000) {
              ...
              const assignedPort = nextDynamicPort++;
...
async function main() {
  ...
  // Start primary Erxiao and Douer engines concurrently
  await startErxiao();
  await startDouer();
}
